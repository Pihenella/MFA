...
**Статус:** Design approved by user (2026-04-27). Ready for writing-plans.
...
- **GDPR/152-ФЗ** — обработка ПД. Для запуска в РФ нужна политика конфиденциальности + согласие на обработку ПД на форме регистрации (чекбокс «Я согласен») + оферта. **Решение Юрия (2026-04-27):** полноценная оферта + политика + чекбокс согласия делаются в подпроекте D вместе с биллингом и Telegram-ботом. В A `/register` без legal-чекбокса. Перед открытием регистрации сторонним юзерам D должен быть выпущен — это записано в pre-launch чек-листе D (не A).
